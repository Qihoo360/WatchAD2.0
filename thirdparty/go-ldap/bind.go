...
func (bindReq *BindRequest) Unmarshal(b []byte) error {
	packet := ber.DecodePacket(b)

	if len(packet.Children) < 2 {
		return fmt.Errorf("bind request packet error")
	}

	bindReq.messageID = packet.Children[0].Value.(int64)

	if len(packet.Children[1].Children) != 3 {
		return fmt.Errorf("bind request packet error")
	}

	bindReq.version = packet.Children[1].Children[0].Value.(int64)
	bindReq.name = packet.Children[1].Children[1].Value.(string)

	if packet.Children[1].Children[2].Identifier.Tag == 0 {
		bindReq.authentication = simple(packet.Children[1].Children[2].Data.String())
	} else {
		bindReq.authentication = saslCredentials{
			mechanism:   packet.Children[1].Children[2].Children[0].Value.(string),
			credentials: packet.Children[1].Children[2].Children[1].Value.(string),
		}
	}
	return nil
}
